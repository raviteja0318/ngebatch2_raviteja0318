...
export const createEmployee = async (Request: Request, response: Response) => {
  ...
  let lastname = "dhanakonda";
  try {
    const empl = await pool.query(
      `insert into employee values(${empno},${firstname},${lastname});`
    );
    console.log("Created Employee: ", empl);
    response.json(`${empl} Employee Created`);
  } catch (error) {
    console.log("Error thrown in createEmployee: ", (error as Error).message);
    response.status(500).json({ message: "There was an error" });
  }
};
...
export const updateEmployee = async (Request: Request, response: Response) => {
  ...
  let empno = 1300;
  let newlastname = 'danakonda';
  try {
    const empl = await pool.query(
      `UPDATE employee set lastname=${newlastname} where empno=${empno};`
    );
    console.log("Updayed Employee:", empl);
    response.json(`${empl} Employee Updated`);
  } catch (error) {
    console.log("Error thrown in updateEmployee:", (error as Error).message);
    response.status(500).json({ message: "There was an error" });
  }
