...
export function countFromEurope (developers: Developer[]) {
       return developers.filter((dev) => dev.country == 'Europe').length; 
}
...
export function getGreetings (developers: Developer[]) {
        return developers.map((dev) => `Hi ${dev.firstName}, what do you like the most about ${dev.language}?`);
}
...
export function isJSComing (developers: Developer[]) {
        const lang = developers.map((dev) => dev.language);
        return lang.includes("Javascript");
}
